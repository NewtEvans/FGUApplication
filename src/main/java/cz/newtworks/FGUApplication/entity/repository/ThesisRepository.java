...
public interface ThesisRepository extends JpaRepository<ThesisEntity, Long>, JpaSpecificationExecutor<ThesisEntity> {
    ...
    long countByPauseDateBetween(LocalDate start, LocalDate end);

    long countByPauseDateBetweenAndThesisType(LocalDate start, LocalDate end, ThesisType thesisType);
    ...
    @Query("SELECT COUNT(e) FROM ThesisEntity e WHERE e.startDate <= :year AND (e.endDate > :year OR e.endDate IS NULL)")
    long countOngoingTheses(LocalDate year);
